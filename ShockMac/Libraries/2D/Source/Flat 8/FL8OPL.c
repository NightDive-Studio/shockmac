...
// 68k globals
int 	g_l_u_mask,g_l_v_mask,g_l_v_shift;
uchar *g_bm_bits;
fix 	g_l_scan_slope;
int		g_gr_row;
...
void gri_opaque_per_umap_hscan_scanline(grs_per_info *pi, grs_bitmap *bm) {
   ...
   if (l_x<pi->xl) {
	...
	if (l_x<pi->xr0)
	 {
	 	g_l_v_shift = l_v_shift;
	 	g_l_u_mask = l_u_mask;
		g_l_v_mask = l_v_mask;
	  g_bm_bits = bm->bits;
	  g_l_scan_slope = l_scan_slope;
	  g_gr_row = grd_bm.row;
		opaque_per_hscan_68K_Loop(pi->xr0-l_x, l_du, l_dv, &l_u, &l_v, &p, &l_y_fix, &y_cint);
		l_x=pi->xr0;
	 }
#endif
...
/*   for (;l_x<l_xr0;l_x++) {
  ...
  movem.l	d0-d7/a0-a6,-(sp)    
	...
	move.l	92(sp),a0
	move.l	(a0),d4			// y_cint
	move.l	g_l_v_shift,d3
	move.l	(a1),a0
	move.l	g_bm_bits,a1
	move.l	g_gr_row,d5
	move.l	g_l_scan_slope,a2
	
@Loop:
	move.l	d1,d6
	swap		d6
	ext.l		d6							// l_u>>16
	and.l		g_l_u_mask,d6		// k = (l_u>>16)&l_u_mask
	move.l	d2,d7
	asr.l		d3,d7						// l_v>>l_v_shift
	and.l		g_l_v_mask,d7		// (l_v>>l_v_shift)&l_v_mask
	add.l		d7,d6						// k+=(l_v>>l_v_shift)&l_v_mask;
	move.b	(a1,d6.l),(a0)+	// *p=bm_bits[k];
	...
	if (l_y<l_yr0)
	 {
	  ...
	  g_l_scan_slope = l_scan_slope;
	  g_gr_row = gr_row;
	 	opaque_per_vscan_68K_Loop(l_yr0-l_y, l_du, l_dv, &l_u, &l_v, &p, &l_x_fix, &x_cint);
	 	l_y  = l_yr0;
	 }
#endif
