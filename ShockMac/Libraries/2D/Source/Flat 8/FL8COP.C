...
void gri_opaque_clut_per_umap_hscan_scanline(grs_per_info *pi, grs_bitmap *bm) {
   ...
   for (;l_x<l_xr0;l_x++) {
	...
	if (l_x<l_xr0)
	 {
	 	oc_l_v_shift = l_v_shift;
	 	oc_l_u_mask = l_u_mask;
		oc_l_v_mask = l_v_mask;
	  oc_bm_bits = bm->bits;
	  oc_l_scan_slope = l_scan_slope;
	  oc_gr_row = grd_bm.row;
	  oc_clut = t_clut;
		opaque_clut_per_hscan_68K_Loop(l_xr0-l_x, l_du, l_dv, &l_u, &l_v, &p, &l_y_fix, &y_cint);
		l_x=l_xr0;
	 }
#endif
...
/*   for (;l_x<l_xr0;l_x++) {
  ...
  movem.l	d0-d7/a0-a6,-(sp)    
	...
	move.l	92(sp),a0
	move.l	(a0),d4			// y_cint
	move.l	oc_l_v_shift,d3
	move.l	(a1),a0
	move.l	oc_bm_bits,a1
	move.l	oc_gr_row,d5
	move.l  oc_clut,a2
	move.l	oc_l_scan_slope,a3
	
@Loop:
	move.l	d1,d6
	swap		d6
	ext.l		d6							// l_u>>16
	and.l		oc_l_u_mask,d6		// k = (l_u>>16)&l_u_mask
	move.l	d2,d7
	asr.l		d3,d7						// l_v>>l_v_shift
	and.l		oc_l_v_mask,d7		// (l_v>>l_v_shift)&l_v_mask
	...
	if (l_y<l_yr0)
	 {
	  ...
	  oc_gr_row = gr_row;
	  oc_clut = t_clut;
	 	opaque_clut_per_vscan_68K_Loop(l_yr0-l_y, l_du, l_dv, &l_u, &l_v, &p, &l_x_fix, &x_cint);
	 	l_y  = l_yr0;
	 }
#endif
